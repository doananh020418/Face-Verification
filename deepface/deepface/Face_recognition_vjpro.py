...
def add_employee(id, img_paths):
    model_name = 'VGG-Face'
    distance_metric = 'cosine'
    foldername = 'data/' + str(id)
    path = os.path.join(os.path.dirname(os.path.abspath(os.getcwd())), foldername)
    ...
    for img_path in img_paths:
        ...
        imp = str(i) + '.jpg'
        save_path = os.path.join(path, imp)
        cv2.imwrite(save_path, img)
        i = i + 1
        employees.append(save_path)
        if len(employees) > 0:
            # model = DeepFace.build_model(model_name)
            print(model_name, " is built")
            input_shape = functions.find_input_shape(model)
        ...
        tmp_df = pd.DataFrame(embeddings, columns=['employee', 'embedding'])
