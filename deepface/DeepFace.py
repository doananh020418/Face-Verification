				...
				for k in metric_names:
					...
					if model_name == 'Ensemble' and j == 'OpenFace' and k == 'euclidean':
						...
						if model_name != 'Ensemble':
							...
							df = df[df["%s_%s" % (j, k)] <= threshold*delta]
...
def stream(db_path = '', model_name ='DeepID', distance_metric = 'cosine', enable_face_analysis = False, source = 0, time_threshold = 2, frame_threshold = 5,delta = 1):
	...
	realtime.analysis(db_path, model_name, distance_metric, enable_face_analysis
						, source = source, time_threshold = time_threshold, frame_threshold = frame_threshold,delta=delta)
